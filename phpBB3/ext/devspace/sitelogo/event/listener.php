...
namespace devspace\sitelogo\event;
...
class listener implements EventSubscriberInterface
{
	 ...
	 * @access public
	 */
	public function __construct(config $config, db_text $config_text, template $template, user $user, string $root_path, array $slconstants)
	{
		$this->config      = $config;
	...
	protected function get_hex_colour($base_colour, $offset)
	{
		...
		if ($this->config['site_logo_header_solid'] || (int) ord(substr($base_colour, 1, 1)) > 57)
		{
	...
	/**
 	 ...
 	 * @access protected
 	 */
	function set_site_logo_url($url)
	{

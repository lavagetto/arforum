...
class main_listener implements EventSubscriberInterface
{
    static public function getSubscribedEvents()
    {
        return array(
            ...
            'core.notification_manager_add_notifications' => 'tapa_notification_manager_add_notifications'
//old ones
    ...
    */
    ...
    public function tapa_submit_post_end($event)
    {
        ...
        if(isset($config['tapatalk_push_key']) && $config['tapatalk_push_key'] != "")
        {
            ...
            $message_parser->message =  $data['data']['message'];
            $message_parser->decode_message();

            $data['data']['message'] = $message_parser->message;
            // #
    ...
    public function tapa_notification_manager_add_notifications($event)
    {
        global $config,$phpbb_root_path;
        if(isset($config['tapatalk_push_key']) && $config['tapatalk_push_key'] != "")
        {
            $data = $event->get_data();
            if(isset($data['notification_type_name']))
            {
                switch($data['notification_type_name'])
                {
                    case 'notification.type.topic_in_queue':
                        {
                            if(!class_exists('TapatalkPush'))
                            {
                                $tapatalk_dir = isset($config['tapatalkdir']) && !empty($config['tapatalkdir']) ? $config['tapatalkdir'] : 'mobiquo';
                            ...
                            $TapatalkPush = new \TapatalkPush();
                            $TapatalkPush->doPushPendingTopic($data);
                            break;
                        }
                    case 'notification.type.post_in_queue':
                        {
                            if(!class_exists('TapatalkPush'))
                            {
                                $tapatalk_dir = isset($config['tapatalkdir']) && !empty($config['tapatalkdir']) ? $config['tapatalkdir'] : 'mobiquo';
                            ...
                            $TapatalkPush = new \TapatalkPush();
                            $TapatalkPush->doPushPendingPost($data);
                            break;
                        }
                }
            }
    ...
    public function tapa_header($event)
    {
        ...
        if($page_type =="topic")
        {
            ...
            $app_sharelink_tid = isset($_GET['t']) ? $_GET['t'] : 0 ;
            $app_sharelink_pid = isset($_GET['p']) ? $_GET['p'] : 0;
        }
        ...
        $smartBannerPath = $phpbb_root_path .  $tapatalk_dir. '/smartbanner/';
    ...
    {
        global $user,$phpbb_root_path,$config, $phpbb_container;
        $param_arr = array();
        $TT_bannerControlData = $phpbb_container->get('config_text')->get('tapatalk_banner_control');
        $TT_bannerControlData = !empty($TT_bannerControlData) ? unserialize($TT_bannerControlData) : false;
        if($TT_bannerControlData !== false){
...
//            $message = preg_replace('/\[emoji(\d+)\]/i', '<IMG src="'.$protocol.'://emoji.tapatalk-cdn.com/emoji\1.png" />', $message);
            ...
            $message = preg_replace('/https?:\/\/cloud\.tapatalk\.com/i',  '//cloud.tapatalk.com', $message);
            $message = preg_replace('/https?:\/\/uploads\.tapatalk-cdn\.com/i',  '//uploads.tapatalk-cdn.com', $message);
            $message = preg_replace('/https?:\/\/images\.tapatalk-cdn\.com/i',  '//images.tapatalk-cdn.com', $message);
